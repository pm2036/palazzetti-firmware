...
local os    = require "os"
local ubus  = require "palazzetti.ubus"
...
function delete_first(self, config, type, option, value)
	self:foreach(config, type, function(s)
		if s[".type"] == type then
			self:delete(config, s[".name"], option)
		end
	end)
end

function set_first(self, config, type, option, value)
	self:foreach(config, type, function(s)
		if s[".type"] == type then
			self:set(config, s[".name"], option, value)
		end
	end)
end
